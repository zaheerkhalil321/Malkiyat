...
const Details = (props: Props) => {
  ...
  const [bidData, setBidData] = useState<HighestOfferDataI>(data.bidData);
  useEffect(() => {
    callHighestBidApi();
  }, []);
  useEffect(() => {
    EventEmitter.on(
      ...
      (message: MainResponse<EventMessageI>) => {
        ...
        callHighestBidApi(parseMessage.message!);
      }
    );
    return () => {
  ...
  async function callHighestBidApi(message: string = data.bidData.message!) {
  ...
  return (
    <Container>
      ...
      <View style={{ paddingHorizontal: wp(5) }}>
        ...
        >
          {bidData?.highestOffer?.map((item, index) => (
            <View key={index}>
              ...
              {bidData?.highestOffer?.length - 1 == index ? null : (
                <View
                  style={{
      ...
      <View style={{ paddingHorizontal: wp(5) }}>
        ...
        >
          {bidData?.bidOffers
            ?.slice(bidData.highestOffer.length)
            ?.map((item, index) => (
              <View key={index}>
                <View
                  style={{
